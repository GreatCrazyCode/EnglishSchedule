...
为了实现最高效的AI口语练习，我们采用“固定指令 + 动态内容”的模式。

每次练习时，您只需要**依次复制【第一部分】和【第二部分】的全部内容**，然后将它们**一次性粘贴**到您想用来练习的AI对话框中即可。

---

### 【第一部分】固定指令 (请复制并永久保存这部分模板)
...
### 【第二部分】动态内容生成规则 (本规则用于指导我)

*这部分内容将由我根据您的学习进度，在每周一和周三生成，文件名为 `SpeakingPractice_Day*.md`。*

1.  文件**必须**以 `# TOPIC AND QUESTIONS FOR TODAY'S SESSION` 作为一级标题开头。
2.  **Topic**: 使用 `## Topic` 作为二级标题，并用一个简洁的英文句子描述话题。
3.  **Key Vocabulary**: 使用 `## Key Vocabulary and Expressions to Practice` 作为二级标题，并列出当天需要练习的英文词汇和短语。
4.  **Practice Questions**: 使用 `## Practice Questions` 作为二级标题，列出5-6个全英文、有层次的练习问题（例如：热身、核心、扩展问题）。
